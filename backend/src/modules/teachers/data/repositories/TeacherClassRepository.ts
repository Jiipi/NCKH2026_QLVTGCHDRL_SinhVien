...
export interface AssignMonitorResult {
  classId: string;
  monitorStudentId: string;
}

/**
 * Teacher Class Repository class
 */
class TeacherClassRepository {
   ...
   * @param teacherId - Teacher's user id (NguoiDung.id)
   * @param classId - Lop.id
   * @param studentId - SinhVien.id
   * @returns Result with classId and monitorStudentId
   */
  async assignClassMonitor(teacherId: string, classId: string, studentId: string): Promise<AssignMonitorResult> {
    ...
    if (!student) {
      const err = new Error('Sinh viên không thuộc lớp này hoặc không tồn tại') as Error & { statusCode?: number };
      err.statusCode = 400;
      throw err;
    }
...
export default TeacherClassRepository;
