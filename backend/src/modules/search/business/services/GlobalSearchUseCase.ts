...
class GlobalSearchUseCase {
  ...
  async execute(query: string | undefined, user: UserContext): Promise<SearchResults> {
    ...
    const activityWhere: { AND: object[] } = { AND: [activityTextFilter] };
    ...
    let myActivitiesWhere: { AND: object[] } | null = null;
    if (userId) {
    ...
    let myActivities: ActivityResult[] = [];
...
export default GlobalSearchUseCase;
