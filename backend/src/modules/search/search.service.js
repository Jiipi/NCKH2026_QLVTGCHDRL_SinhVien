...
class SearchService {
  /**
   * Global search across activities, students, classes, teachers
   * @param {string} query - Search query
   * @param {Object} user - Current user info
   * @returns {Promise<Object>} Search results
   */
  static async globalSearch(query, user) {
    try {
      const userId = user?.id || user?.sub || null;
      const userRole = normalizeRoleName(user?.vai_tro?.ten_vt || user?.role || '');

      logInfo('Global search', { query, userId, userRole });
      ...
      if ((userRole === 'SINH_VIEN' || userRole === 'LOP_TRUONG') && userId) {
        studentRecord = await searchRepo.getStudentByUserId(userId);
        
        if (studentRecord?.lop_id) {
          // Get list of creators from class
          const classCreators = await searchRepo.getClassCreators(studentRecord.lop_id);
          const teacherClass = await searchRepo.getClassHomeroom(studentRecord.lop_id);
      ...
      const baseActivities = await searchRepo.searchActivities(activityWhere, { take: 5 });

      // Query "My Activities" (if applicable)
      let myActivities = [];
      if (myActivitiesWhere) {
        myActivities = await searchRepo.searchActivities(myActivitiesWhere, { take: 5 });
      }
      ...
      if (userRole === 'ADMIN' || userRole === 'GIANG_VIEN' || userRole === 'LOP_TRUONG') {
        ...
        if (userRole === 'GIANG_VIEN' && userId) {
          const teacherClasses = await searchRepo.getTeacherClasses(userId);
          studentWhere.lop_id = { in: teacherClasses.map(c => c.id) };
        } else if (userRole === 'LOP_TRUONG' && userId) {
          const monitor = await searchRepo.getStudentByUserId(userId);
          if (monitor?.lop_id) {
            studentWhere.lop_id = monitor.lop_id;
          }
        }

        results.students = await searchRepo.searchStudents(studentWhere, { take: 5 });
      }
      ...
      if (userRole === 'ADMIN' || userRole === 'GIANG_VIEN') {
        ...
        results.classes = await searchRepo.searchClasses(classWhere, { take: 5 });
      }
      ...
      if (userRole === 'ADMIN') {
        ...
        results.teachers = await searchRepo.searchTeachers(teacherWhere, { take: 5 });
      }
