...
/**
 * Input data structure for UpdateActivityDto
 */
...
/**
 * Domain data structure for activity update (partial)
 */
interface UpdateActivityDomain {
  ten_hd?: string;
  mo_ta?: string | null;
...
class UpdateActivityDto {
  ...
  static fromRequest(body: UpdateActivityInput): UpdateActivityDto {
    return new UpdateActivityDto(body);
  }

  /**
   * Convert to domain object, only including defined fields
   * @returns Domain object with only defined fields
   */
  toDomain(): UpdateActivityDomain {
    ...
    const result: UpdateActivityDomain = {};
