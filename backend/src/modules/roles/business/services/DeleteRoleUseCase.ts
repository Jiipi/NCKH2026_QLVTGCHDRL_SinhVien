...
export interface DeleteRoleOptions {
  reassignTo?: string;
  cascadeUsers?: boolean | string;
}

interface ValidationErrorWithCount extends ValidationError {
  usersCount?: number;
}
...
class DeleteRoleUseCase {
  ...
  async execute(roleId: string, options: DeleteRoleOptions = {}): Promise<boolean> {
    ...
    if (usersCount > 0 && !reassignTo && !cascadeUsers) {
      const error = new ValidationError('Vai trò đang được sử dụng') as ValidationErrorWithCount;
  ...
  private async _cascadeDeleteUsers(roleId: string): Promise<void> {
