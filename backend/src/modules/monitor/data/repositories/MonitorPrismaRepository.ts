...
class MonitorPrismaRepository extends IMonitorRepository {
  async findStudentsByClass(classId: string): Promise<StudentWithRelations[]> {
  ...
  async findStudentRegistrations(studentId: string, activityFilter?: ActivityFilter): Promise<RegistrationWithRelations[]> {
  ...
  async findClassRegistrationsForPoints(classId: string, activityFilter?: ActivityFilter): Promise<RegistrationWithRelations[]> {
    return await prisma.dangKyHoatDong.findMany({
      where: {
  ...
  async findClassRegistrations(classId: string, filters: RegistrationFilters = {}): Promise<RegistrationWithRelations[]> {
  ...
  async countPendingRegistrations(classId: string): Promise<number> {
  ...
  async findRegistrationById(registrationId: string): Promise<RegistrationWithRelations | null> {
  ...
  async updateRegistrationStatus(registrationId: string, status: TrangThaiDangKy | string, additionalData: Record<string, unknown> = {}): Promise<DangKyHoatDong> {
    return await prisma.dangKyHoatDong.update({
      where: { id: registrationId },
      data: {
        trang_thai_dk: status,
        ngay_duyet: new Date(),
        nguoi_duyet_id: additionalData.nguoi_duyet_id as string || null,
        ...additionalData
      }
    });
  }

  async createNotification(data: NotificationData): Promise<ThongBao> {
    return await prisma.thongBao.create({ data });
  }

  async findNotificationTypeByName(name: string): Promise<LoaiThongBao | null> {
  ...
  async countStudentsByClass(classId: string): Promise<number> {
    return await prisma.sinhVien.count({
      where: { lop_id: classId }
    });
  }

  async countRegistrations(classId: string, filters: RegistrationFilters = {}): Promise<number> {
    const { status, activityFilter } = filters;
  ...
  async findRecentRegistrations(classId: string, activityFilter?: ActivityFilter, limit: number = 5): Promise<RegistrationWithRelations[]> {
    return await prisma.dangKyHoatDong.findMany({
      where: {
        sinh_vien: { lop_id: classId },
        hoat_dong: activityFilter && Object.keys(activityFilter).length ? { is: activityFilter } : undefined
  ...
  async findUpcomingActivities(classId: string, activityFilter?: ActivityFilter, limit: number = 5): Promise<ActivityWithRelations[]> {
    const now = new Date();
  ...
  async findClassById(classId: string): Promise<ClassWithRelations | null> {
  ...
  async findAllStudentsInClass(classId: string): Promise<StudentWithRelations[]> {
    return await prisma.sinhVien.findMany({
      where: { lop_id: classId },
      include: {
        nguoi_dung: { 
  ...
  async findClassRegistrationsForCountApproved(classId: string, activityFilter?: ActivityFilter): Promise<{ hd_id: string }[]> {
    return await prisma.dangKyHoatDong.findMany({
      where: {
        sinh_vien: { lop_id: classId },
  ...
  async countActivitiesForClassStrict(classId: string, semesterWhere?: SemesterWhere): Promise<number> {
    const classStudents = await this.findAllStudentsInClass(classId);
    const classCreatorUserIds: string[] = classStudents.map(s => s.nguoi_dung_id).filter(Boolean) as string[];
  ...
  async findClassRegistrationsForReports(classId: string, activityFilter?: ActivityFilter): Promise<RegistrationWithRelations[]> {
