 ...
 */
interface UseCases {
  getSemesterOptions: { execute(): Promise<unknown> };
  getCurrentSemester: { execute(): Promise<unknown> };
  getSemesterStatus: { execute(classId: string, semester: string): unknown };
  getCurrentSemesterStatus: { execute(classId: string | null, userId: string | null, classMonitor: unknown, semester: string | null): Promise<unknown> };
  proposeClosure: { execute(classId: string, actorId: string, semester: string): Promise<unknown> };
  softLock: { execute(classId: string, actorId: string, semester: string, graceHours: number): Promise<unknown> };
  hardLock: { execute(classId: string, actorId: string, semester: string): Promise<unknown> };
  rollback: { execute(classId: string, actorId: string, semester: string): Promise<unknown> };
  getAllClasses: { execute(): Promise<unknown> };
  getClassDetail: { execute(classId: string): Promise<unknown> };
  getClassStudents: { execute(classId: string): Promise<unknown> };
  getActivitiesBySemester: { execute(classId: string, semester: string): Promise<unknown> };
  getRegistrationsBySemester: { execute(classId: string, semester: string): Promise<unknown> };
  createNextSemester: { execute(user: unknown): Promise<{ success: boolean; data?: unknown; message: string }> };
  activateSemester: { execute(semester: string, user: unknown): Promise<{ success: boolean; data?: unknown; message: string }> };
}
...
class SemestersController {
  ...
  async getSemesterOptions(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async getCurrentSemester(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async getSemesterStatus(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async getCurrentSemesterStatus(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const classId = (req.query?.classId as string) || null;
      const semester = (req.query?.semester as string) || null;
  ...
  async proposeClosure(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async softLock(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async hardLock(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { classId, semester } = req.body;
      const actorId = req.user!.sub;
  ...
  async rollback(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { classId, semester } = req.body;
      const actorId = req.user!.sub;
  ...
  async getAllClasses(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async getClassDetail(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
  ...
  async getActivitiesBySemester(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { classId, semester } = req.params;
  ...
  async getRegistrationsBySemester(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { classId, semester } = req.params;
  ...
  async createNextSemester(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Không thể tạo học kỳ mới';
      return sendResponse(res, 500, ApiResponse.error(errorMessage));
    }
  }

  async activateSemester(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const userRole = req.user?.role || req.user?.vai_tro;
      if (!userRole || !['ADMIN', 'QUẢN_TRỊ_VIÊN'].includes(userRole.toUpperCase().replace(/\s/g, '_'))) {
        return sendResponse(res, 403, ApiResponse.error('Không có quyền truy cập'));
      ...
      const errorMessage = error instanceof Error ? error.message : 'Không thể kích hoạt học kỳ';
      return sendResponse(res, 500, ApiResponse.error(errorMessage));
    }
  }
}
