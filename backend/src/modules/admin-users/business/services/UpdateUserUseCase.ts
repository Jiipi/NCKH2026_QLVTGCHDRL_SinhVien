...
import type { IAdminUserRepository, UserRecord, SinhVienRecord } from '../interfaces/IAdminUserRepository';
import type { IHashService } from '../interfaces/IHashService';
import type UpdateUserDto from '../dto/UpdateUserDto';
import type { StudentUpdateData } from '../validators/admin-users.validators';
...
class UpdateUserUseCase {
  ...
  async execute(userId: string, dto: UpdateUserDto, adminId: string): Promise<UpdateUserResult> {
  ...
  private async createStudentProfile(userId: string, studentData: StudentUpdateData): Promise<SinhVienRecord> {
    ...
    const ngaySinh = studentData.ngay_sinh 
      ? (studentData.ngay_sinh instanceof Date ? studentData.ngay_sinh : new Date(studentData.ngay_sinh))
      : new Date();
  ...
  private async updateStudentProfile(studentId: string, studentData: StudentUpdateData): Promise<void> {
    ...
    if (studentData.ngay_sinh) {
      updatePayload.ngay_sinh = studentData.ngay_sinh instanceof Date 
        ? studentData.ngay_sinh 
        : new Date(studentData.ngay_sinh);
    }
