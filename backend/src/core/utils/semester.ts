 ...
 *
 * @module core/utils/semester
 */

/**
 * Semester information interface
 */
export interface SemesterInfo {
  semester: 'hoc_ky_1' | 'hoc_ky_2';
...
/**
 * Activity interface for semester checking
 */
export interface ActivityWithDate {
  ngay_bd?: Date | string | null;
  [key: string]: unknown;
}
 ...
 */
export type SemesterWhereClause = {
  hoc_ky?: string;
  nam_hoc?: string;
  ngay_bd?: {
    gte?: Date;
 ...
 * @param semesterStr - Input semester string
 * @returns Normalized format or null if invalid
 */
export function normalizeSemesterFormat(semesterStr: string | null | undefined): string | null {
 ...
 * @param hocKy - 'hoc_ky_1' or 'hoc_ky_2'
 * @param year - Year as YYYY
 * @returns Format: hoc_ky_X_YYYY
 */
export function buildSemesterValue(hocKy: string, year: string | number): string {
 ...
 * @param date - Ngày cần xác định học kỳ
 * @returns { semester, yearLabel, year, value }
 */
export function determineSemesterFromDate(date: Date): SemesterInfo {
  const month = date.getMonth() + 1; // 1-12
  const year = date.getFullYear();

  let semester: 'hoc_ky_1' | 'hoc_ky_2';
 ...
 * @param semesterStr - Format: 'hoc_ky_1_2025' (chuẩn) hoặc 'hoc_ky_1-2025' (legacy)
 * @returns { semester, year, value } hoặc null nếu invalid
 */
export function parseSemesterString(semesterStr: string | null | undefined): SemesterInfo | null {
  ...
  if (semesterOnlyMatch) {
    ...
    return {
      semester: `hoc_ky_${semesterNum}` as 'hoc_ky_1' | 'hoc_ky_2',
      year: null,
      yearLabel: null,
      value: null
    };
 ...
 *
 * @param semesterStr - Semester query string hoặc 'current'
 * @param useDynamicFilter - Nếu true, filter theo ngay_bd thay vì hoc_ky
 * @returns Prisma where clause
 */
export function buildSemesterFilter(
  semesterStr: string | null | undefined,
 ...
 * @param activity - Hoạt động với ngay_bd
 * @param targetSemester - 'hoc_ky_1' hoặc 'hoc_ky_2'
 * @param targetYear - Năm (YYYY)
 * @returns boolean
 */
export function isActivityInSemester(
  ...
  const activityDate = new Date(activity.ngay_bd as Date | string);
 ...
 *
 * @param semesterStr - Format: 'hoc_ky_1-2025'
 * @returns Prisma where clause
 */
export function buildRobustActivitySemesterWhere(
  semesterStr: string | null | undefined
): SemesterWhereClause {
 ...
 * @param sem1 - First semester string
 * @param sem2 - Second semester string
 * @returns True if they represent the same semester
 */
export function isSameSemester(
  sem1: string | null | undefined,
  sem2: string | null | undefined
): boolean {
