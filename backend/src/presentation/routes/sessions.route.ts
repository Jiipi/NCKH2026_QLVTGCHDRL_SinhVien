 ...
 * @module presentation/routes/sessions
 */

import { Router, Request, Response, NextFunction } from 'express';
import SessionTrackingService from '../../business/services/session-tracking.service';
 ...
 */
router.get('/active-users', auth, async (req: AuthRequest, res: Response) => {
  try {
 ...
 * @access  Private
 */
router.get('/my-sessions', auth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    logError('Failed to get user sessions', error as Error);
    return sendResponse(
      res,
      500,
...
router.get('/status/:userId', auth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    logError('Failed to get user activity status', error as Error);
    return sendResponse(
      res,
      500,
 ...
 * @access  Private
 */
router.post('/heartbeat', auth, async (req: AuthRequest, res: Response) => {
  try {
    const tabId = req.headers['x-tab-id'] as string || req.body.tabId;
    const userId = req.user?.sub;
    ...
    logError('Failed to update heartbeat', error as Error);
    return sendResponse(
      res,
      500,
 ...
 * @access  Private
 */
router.delete('/logout', auth, async (req: AuthRequest, res: Response) => {
  try {
    const tabId = req.headers['x-tab-id'] as string || req.body.tabId;
    ...
    logError('Failed to remove session', error as Error);
    return sendResponse(
      res,
      500,
