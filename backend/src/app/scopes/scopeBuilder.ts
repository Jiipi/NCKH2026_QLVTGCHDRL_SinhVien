 ...
 * - SINH_VIEN: Chỉ thấy data của lớp mình
 *   - Nếu isMonitor = true: Có thêm quyền điểm danh và quản lý lớp
 * 
...
async function buildScope(resource: string, user: ScopeUser): Promise<ScopeClause> {
  ...
  try {
    switch (normalizedRole) {
      ...
      case 'SINH_VIEN':
        return await buildStudentScope(resource, userId, false);

      case 'LOP_TRUONG':
        return await buildStudentScope(resource, userId, true);
...
/**
 * Build scope for SINH_VIEN (students and class monitors)
 * They only see data from their own class
 * @param resource - Resource name
 * @param userId - User ID
 * @param isMonitor - Whether user is a class monitor (has additional permissions)
 * @returns Scope clause for Prisma
 */
async function buildStudentScope(resource: string, userId: string, isMonitor: boolean = false): Promise<ScopeClause> {
  ...
  switch (resource) {
    case 'activities':
      ...
      // OR activities assigned to their class (lop_id match)
      return {
        OR: [
          { lop_id: lop_id },                          // Activities assigned to student's class
          { nguoi_tao_id: { in: classCreatorUserIds } } // OR created by classmates/teacher
        ]
      };
...
async function canAccessItem(resource: string, itemId: string, user: ScopeUser): Promise<boolean> {
  ...
  try {
    switch (resource) {
      case 'activities':
        // Direct query without scope - query activity directly
        const activity = await prisma.hoatDong.findUnique({
          where: { id: itemId },
          select: { id: true, nguoi_tao_id: true, lop_id: true }
        });
        ...
        // Check ownership: activity.nguoi_tao_id === userId
        if (activity.nguoi_tao_id === userId) {
          return true;
        }

        // Check teacher access: query lop table for chu_nhiem
        if (normalizedRole === 'GIANG_VIEN' && activity.lop_id) {
          const classInfo = await prisma.lop.findUnique({
            where: { id: activity.lop_id },
            select: { chu_nhiem: true }
          });
        ...
        // Check monitor access: query sinh_vien for lop_id match
        if (normalizedRole === 'LOP_TRUONG' && activity.lop_id) {
          const student = await prisma.sinhVien.findUnique({
            where: { nguoi_dung_id: userId },
            select: { lop_id: true }
          });

          if (student?.lop_id === activity.lop_id) {
            return true;
          }
        }
      ...
      case 'registrations':
        ...
        // Check ownership
        if (registration.sinh_vien?.nguoi_dung_id === userId) {
          return true;
        }

        // Check teacher access
        if (normalizedRole === 'GIANG_VIEN' && registration.sinh_vien?.lop_id) {
          const classInfo = await prisma.lop.findUnique({
            where: { id: registration.sinh_vien.lop_id },
            select: { chu_nhiem: true }
          });
          if (classInfo?.chu_nhiem === userId) {
            return true;
        ...
        if (normalizedRole === 'LOP_TRUONG' && registration.sinh_vien?.lop_id) {
          const student = await prisma.sinhVien.findUnique({
            where: { nguoi_dung_id: userId },
            select: { lop_id: true }
          });

          if (student?.lop_id === registration.sinh_vien.lop_id) {
            return true;
          }
        }
