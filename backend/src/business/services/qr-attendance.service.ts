...
/**
 * Service for QR-based attendance tracking
 */
class QrAttendanceService {
  /**
   * Scan QR code and record attendance
   * @param qrCode - QR code data (JSON string)
   * @param user - User object with id, role, sub
  ...
  async scanQrCode(qrCode: string, user: UserInfo, req: Request): Promise<AttendanceResult> {
    ...
    if (!activity) {
      ...
      const error: HttpError = new Error('Không tìm thấy hoạt động');
      error.status = 404;
      throw error;
    }
    ...
    if (now.getTime() < start.getTime()) {
      ...
      const error: HttpError = new Error(`Hoạt động chưa bắt đầu. Thời gian bắt đầu: ${startDateStr}`);
      error.status = 400;
      throw error;
    }
    ...
    if (now.getTime() > end.getTime()) {
      ...
      const error: HttpError = new Error(`Hoạt động đã kết thúc lúc ${endDateStr}, không thể điểm danh`);
      error.status = 400;
      throw error;
    }
    ...
    if (role !== 'SINH_VIEN' && role !== 'LOP_TRUONG') {
      ...
      const error: HttpError = new Error('Chỉ sinh viên mới được điểm danh bằng QR');
      error.status = 403;
      throw error;
    }
    ...
    if (!sv) {
      ...
      const error: HttpError = new Error('Không tìm thấy thông tin sinh viên của bạn');
      error.status = 400;
      throw error;
    }
    ...
    if (!reg) {
      ...
      const error: HttpError = new Error('Bạn chưa đăng ký hoạt động này. Vui lòng đăng ký trước khi điểm danh.');
      error.status = 403;
      throw error;
    }
    ...
    if (reg.trang_thai_dk === 'tu_choi') {
      ...
      const error: HttpError = new Error('Đăng ký của bạn đã bị từ chối. Không thể điểm danh.');
      error.status = 403;
      throw error;
    }
    ...
    if (existed) {
      ...
      const error: HttpError = new Error(`Bạn đã điểm danh trước đó vào lúc ${attendanceTime}`);
      error.status = 400;
      throw error;
    }
    ...
    const formatDate = (d: Date | null): string => d ? new Date(d).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' }) : '';
