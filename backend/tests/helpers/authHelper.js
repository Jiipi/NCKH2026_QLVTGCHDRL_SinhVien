...
async function createUserWithRole(data = {}, roleName = 'SINH_VIEN') {
  ...
  const userData = {
    ten_dn: data.ten_dn || `test_user_${timestamp}`,
    ho_ten: data.ho_ten || 'Test User',
    email: data.email || `test_${timestamp}@dlu.edu.vn`,
...
async function createStudentUser(data = {}) {
  ...
  let lop = await prisma.lop.findFirst({
    where: { ten_lop: data.lopName || 'DHKTPM17A' }
  });

  if (!lop) {
    // Create admin for chu_nhiem
    ...
    lop = await prisma.lop.create({
      data: {
        ten_lop: data.lopName || 'DHKTPM17A',
  ...
  const sinhVien = await prisma.sinhVien.create({
    data: {
      ...
      mssv: data.mssv || `SV${String(Date.now()).slice(-7)}`, // Max 10 chars
