...
export interface DetailedSessionsInfo {
...
export type SessionEventCallback = (event: CustomEvent<SessionEvent>) => void;

// ==================== SESSION MANAGER CLASS ====================

class SessionManager {
  private sessionId: string;
  private readonly storageKey: string = 'multi_session_data';
  private readonly eventsKey: string = 'multi_session_events';
  private readonly syncKey: string = 'multi_session_sync';
  private isActive: boolean = true;
  private heartbeatInterval: ReturnType<typeof setInterval> | null = null;
  private lastSyncData: SessionData | null = null;
  ...
  private clearLocalStorageForCurrentSession(): void {
    try {
  ...
  // Emit session event
  private emitSessionEvent(eventType: SessionEventType, data: unknown): void {
    try {
  ...
  public onSessionEvent(callback: SessionEventCallback): void {
    window.addEventListener('multiSessionEvent', callback as EventListener);
  }

  // Hủy lắng nghe session events
  public offSessionEvent(callback: SessionEventCallback): void {
    window.removeEventListener('multiSessionEvent', callback as EventListener);
  }
  ...
  public getDetailedSessionsInfo(): DetailedSessionsInfo {
    const activeSessions = this.getActiveSessions();
    const events = this.getSessionEvents();
  ...
  public checkSessionHealth(): boolean {
    const currentSession = this.getSessionData();
