...
const upgradeToHDQuality = (raw: string | null | undefined): string => {
  let url = raw || '';
  if (!url) return url;
...
// Type definitions for avatar functions
 ...
 */
export const getUserAvatar = (userOrStudent: UserAvatarFields | null | undefined, fallbackText: string | null = null): AvatarResult => {
  // Graceful default
  if (!userOrStudent) {
 ...
 */
export const getStudentAvatar = (student: UserAvatarFields | null | undefined): AvatarResult => {
  if (!student) {
    return {
      src: null,
      alt: 'Student Avatar',
