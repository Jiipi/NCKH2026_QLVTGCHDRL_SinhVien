...
function getCurrentRole(): string | null {
  try {
    ...
    return session?.role || session?.user?.vai_tro?.ten_vt || null;
  } catch (_) {
    return null;
  }
}
...
export default function useSemesterData(semesterValue?: string | null, options: UseSemesterDataOptions = {}) {
  const { autoFetchStatus = true } = options;
  const [semesterOptions, setSemesterOptions] = useState<SemesterOption[]>(() => loadInitialOptions());
  const [currentSemester, setCurrentSemester] = useState<string | null>(() => loadInitialCurrentSemester());
  const [loadingOptions, setLoadingOptions] = useState<boolean>(() => semesterOptions.length === 0);
  const [optionsError, setOptionsError] = useState<string>('');

  const [statusCache, setStatusCache] = useState<Record<string, SemesterStatus | '__error__'>>(() => loadInitialStatusCache());
  const [statusLoading, setStatusLoading] = useState<Record<string, boolean>>({});
  const [statusError, setStatusError] = useState<Record<string, string>>({});

  const persistStatusCache = useCallback((nextCache: Record<string, SemesterStatus | '__error__'>) => {
  ...
  const saveOptionsCache = useCallback((nextOptions: SemesterOption[]) => {
  ...
  }, [semesterOptions.length, saveOptionsCache, saveCurrentSemester]);
  ...
  const status = (rawStatus as unknown) === '__error__' ? null : rawStatus;
  ...
  const filteredOptions = useMemo(() => {
    ...
    return semesterOptions.filter(opt => opt.value !== '' && opt.value !== null && opt.value !== undefined);
  }, [semesterOptions, showAllOption]);

  return {
    options: filteredOptions, // Filtered by role
    allOptions: semesterOptions, // Raw options (for admin use)
    currentSemester,
