...
export const useMultiSession = () => {
    ...
    try {
      ...
      if (sessionData) {
        setAuth({
          ...
          user: sessionData.user ? { id: String(sessionData.user.id), role: sessionData.role ?? undefined } : null,
  ...
  useEffect(() => {
    const handleSessionSync = (event: CustomEvent) => {
      ...
      if (sessionData && sessionData.sessionId !== sessionManager.getSessionId()) {
    ...
    const handleMultiSessionEvent = (event: CustomEvent) => {
