...
interface ConfirmModal extends ConfirmOptions {
  onConfirm: () => void;
  onCancel: () => void;
}
...
export function NotificationProvider({ children }: NotificationProviderProps): React.JSX.Element {
  ...
  const addNotification = useCallback((notification: Partial<Notification>): number => {
  ...
  const showSuccess = useCallback((message: string, title: string = 'Thành công', duration: number = 8000): number => {
    return addNotification({ type: 'success', title, message, duration });
  }, [addNotification]);

  const showError = useCallback((message: string, title: string = 'Lỗi', duration: number = 6000): number => {
    return addNotification({ type: 'error', title, message, duration });
  }, [addNotification]);

  const showWarning = useCallback((message: string, title: string = 'Cảnh báo', duration: number = 5000): number => {
    return addNotification({ type: 'warning', title, message, duration });
  }, [addNotification]);

  const showInfo = useCallback((message: string, title: string = 'Thông tin', duration: number = 4000): number => {
...
function NotificationContainer(): React.JSX.Element | null {
