...
export default function Pagination({ pagination, onPageChange, className = '' }) {
  ...
  return (
    <div className={`flex items-center justify-between border-t border-gray-200 bg-white px-3 py-3 dark:border-slate-700 dark:bg-slate-900 sm:px-6 ${className}`}>
      <div className="flex flex-1 items-center justify-between gap-2 sm:hidden">
        {/* Mobile pagination */}
        <button
          onClick={() => handlePageChange(prevPage)}
          disabled={!hasPrevPage}
          className="touch-target relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
        >
          Trước
        </button>
        <span className="min-w-0 truncate px-2 text-center text-sm font-semibold text-gray-700 dark:text-slate-200">
