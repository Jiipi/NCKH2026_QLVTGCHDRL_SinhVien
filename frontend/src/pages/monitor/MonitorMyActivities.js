  ...
  function getFilteredRegistrations() {
    ...
    if (filters.type) {
      filtered = filtered.filter(reg => {
        ...
        const activityType = typeof activity.loai === 'string' 
          ? activity.loai 
    ...
    return (
      <div className={displayViewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4' : 'space-y-3'}>
        {filteredRegs.map(reg => {
          ...
          if (displayViewMode === 'list') {
            return (
              <div key={reg.id} className="group relative">
                <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-500 rounded-xl blur opacity-10 group-hover:opacity-20 transition-opacity duration-200"></div>
                  ...
                  <div className="flex items-stretch gap-4 p-4">
                    ...
                    <div className="flex flex-col justify-center gap-2 flex-shrink-0">
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 font-medium text-sm shadow-md hover:shadow-lg transition-all duration-200 whitespace-nowrap min-w-[90px]"
                      >
          ...
          // GRID MODE - Compact vertical layout matching student page
          return (
            <div key={reg.id} className="group relative h-full">
              <div className="absolute inset-0 bg-gradient-to-br from-purple-400 to-pink-500 rounded-xl blur opacity-5 group-hover:opacity-10 transition-opacity duration-300"></div>
