...
import { 
  Users, Calendar, Clock, Activity, AlertCircle, Trophy, MapPin, Filter, Zap, Target, Star, Sparkles
} from 'lucide-react';
...
export default function MonitorDashboard() {
  ...
  const { summary, upcomingActivities, recentApprovals, topStudents } = dashboard;
  const progressPercent = Math.min(Math.round((summary.avgClassScore / 100) * 100), 100);
  
  // Calculate total training points for monitor (can be class total or monitor's own points)
  // For now, using class average * total students as an approximation
  const totalClassPoints = Math.round((summary.avgClassScore || 0) * (summary.totalStudents || 1));
  const totalPointsProgress = Math.min((totalClassPoints / (100 * (summary.totalStudents || 1))) * 100, 100);
  
  // Calculate additional stats for cards
  const activitiesJoined = summary.activitiesJoined || 0; // Activities that students have joined
  const classRank = summary.monitorRank || summary.classRank || 1; // Monitor's rank in class (if applicable)
  const goalPoints = summary.goalPoints || 0;
  const goalText = summary.goalText || '';
  
  // Get classification based on class average score
  ...
  const classification = getClassification(summary.avgClassScore || 0);
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  {/* Badges - Xếp loại lớp, Tên lớp, Số sinh viên */}
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="px-3 py-1 rounded-lg text-xs font-bold bg-blue-100 text-blue-700 border border-blue-300">
                      {summary.totalStudents} sinh viên
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="col-span-2 group relative">
                ...
                <div className="relative bg-gradient-to-br from-pink-400 via-purple-500 to-purple-600 border-4 border-black p-3 rounded-xl transform transition-all duration-300 group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 h-full">
                  ...
                  <div className="flex items-baseline gap-1">
                    <p className="text-4xl font-black text-white">{formatNumber(totalClassPoints)}</p>
                    <p className="text-sm font-bold text-white/70">/{(summary.totalStudents || 1) * 100}</p>
                  </div>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-blue-400 border-4 border-black rounded-xl p-3 transform transition-all duration-300 group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 h-full flex flex-col">
                  ...
                  <p className="text-3xl font-black text-white mb-0.5">{summary.totalStudents}</p>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-orange-400 border-4 border-black rounded-xl p-3 transform transition-all duration-300 group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 h-full flex flex-col">
                  ...
                  <p className="text-3xl font-black text-black mb-0.5">{summary.pendingApprovals}</p>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-purple-400 border-4 border-black rounded-xl p-3 transform transition-all duration-300 group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 h-full flex flex-col">
                  ...
                  <p className="text-3xl font-black text-white mb-0.5">{summary.totalActivities}</p>
                  <p className="text-[10px] font-black text-white/80 uppercase tracking-wider">HOẠT ĐỘNG</p>
                </div>
              </div>
              ...
              <div className="group relative">
                ...
                <div className="relative bg-blue-500 border-4 border-black rounded-xl p-3 transform transition-all duration-300 group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 h-full flex flex-col">
                  ...
                  <p className="text-3xl font-black text-white mb-0.5">{classRank}/{summary.totalStudents}</p>
                  ...
                  recentApprovals && recentApprovals.length > 0 ? (
                    recentApprovals.map(activity => (
                      <div 
                        key={activity.id}
                        onClick={() => handleActivityClick(activity.id)}
                        className="group/item cursor-pointer bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200"
                      >
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="font-bold text-gray-900 flex-1 text-sm leading-tight">{activity.ten_hd}</h3>
                          <span className="bg-purple-600 text-white px-2.5 py-1 rounded-full text-xs font-bold ml-2 whitespace-nowrap">
                            +{activity.diem_rl} điểm
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-gray-700 font-medium">
                          <span className="flex items-center gap-1">
                            <Clock className="h-3.5 w-3.5" />
                            {new Date(activity.ngay_bd || activity.ngay_kt).toLocaleDateString('vi-VN')}
                          </span>
                          {(activity.dia_diem || activity.dia_diem_hd) && (
                            <span className="flex items-center gap-1">
                              <MapPin className="h-3.5 w-3.5" />
                              {activity.dia_diem || activity.dia_diem_hd}
                            </span>
