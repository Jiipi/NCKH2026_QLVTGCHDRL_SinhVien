  ...
  React.useEffect(() => {
    const loadClasses = async () => {
      try {
        ...
        // Try to get faculties from dedicated endpoint for completeness
        try {
        ...
        setAllClasses([]);
        setClasses([]);
        // Attempt to load faculties even if classes failed
        try {
          const fres = await http.get('/auth/faculties');
          const fdata = fres.data?.data || fres.data;
          const list = Array.isArray(fdata)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data || res.data?.success) {
        ...
        // Delay navigation để user có thể đọc thông báo
        setTimeout(() => {
          navigate('/login');
        }, 1500);
      }
      ...
      // Prefer field-level errors from backend so người dùng biết rõ lỗi gì
      if (Array.isArray(validationErrors) && validationErrors.length) {
        ...
        for (const e of validationErrors) {
          ...
          // Map backend field name -> our form state keys
          const keyMap = {
            name: 'name',
            maso: 'maso',
            email: 'email',
        ...
        // Hiển thị toast error với danh sách lỗi
        showError(
