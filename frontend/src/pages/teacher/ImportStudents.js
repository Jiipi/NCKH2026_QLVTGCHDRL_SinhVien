  ...
  const downloadTemplate = async () => {
    try {
      ...
      try {
        ...
        const list = clsRes?.data?.data?.classes || clsRes?.data?.data || [];
        if (Array.isArray(list) && list.length > 0) {
          // If classId from URL, find that specific class; otherwise use first
          if (classIdFromURL) {
            const selectedClass = list.find(c => String(c.id) === String(classIdFromURL));
