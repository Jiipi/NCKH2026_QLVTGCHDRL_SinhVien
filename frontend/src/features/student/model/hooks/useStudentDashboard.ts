...
/** Interface for tong_quan statistics */
interface TongQuanStats {
  tong_diem?: number;
  muc_tieu?: number;
  tong_hoat_dong?: number;
}

/** Interface for class comparison stats */
interface SoSanhLopStats {
  my_rank_in_class?: number;
  total_students_in_class?: number;
}

/** Interface for dashboard data from mapDashboardToUI */
interface StudentDashboard {
  hoat_dong_sap_toi: unknown[];
  tong_quan: TongQuanStats;
  so_sanh_lop: SoSanhLopStats;
  sinh_vien: Record<string, unknown>;
  hoat_dong_gan_day?: unknown[];
...
export default function useStudentDashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      } else if (!dashboardResult.success && 'error' in dashboardResult) {
      ...
      } else if (!profileResult.success && 'error' in profileResult) {
  ...
  const dashboard = useMemo((): StudentDashboard => {
    if (!dashboardData) {
      return {
        hoat_dong_sap_toi: [],
    ...
    return mapDashboardToUI(dashboardData) as StudentDashboard;
  }, [dashboardData]);
  ...
  const summary = useMemo(() => {
    const tongQuan: TongQuanStats = dashboard.tong_quan || {};
    const soSanhLop: SoSanhLopStats = dashboard.so_sanh_lop || {};
