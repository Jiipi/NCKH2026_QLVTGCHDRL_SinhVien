  ...
  return (
    ...
    >
      <div className="rounded-[2rem] border border-white/60 bg-white/60 p-4 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/55 dark:shadow-black/20">
        <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
      ...
      <div className="rounded-[2rem] border border-white/60 bg-white/50 shadow-[0_18px_60px_rgba(15,23,42,0.06)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/45 dark:shadow-black/20">
        <div className="grid grid-cols-2 divide-x divide-y divide-slate-200/70 dark:divide-white/10 sm:flex sm:flex-wrap sm:items-stretch sm:divide-y-0">
          {tabs.map((config) => (
            <StatusTab
              key={config.key}
              icon={config.icon}
...
function StatusTab({ icon: Icon, label, count, active, onClick, colorKey }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`touch-target relative flex min-w-0 flex-1 cursor-pointer items-center gap-2.5 px-3 py-3 transition-all duration-200 sm:min-w-[140px] sm:px-4 ${
