...
interface UseAdminActivitiesReturn {
  ...
  semesterValue: string | null;
  setSemesterValue: React.Dispatch<React.SetStateAction<string | null>>;
  filtered: Activity[];
  reload: () => Promise<void>;
}

export default function useAdminActivities(): UseAdminActivitiesReturn {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const arr = (data as { activityTypes?: ActivityType[]; items?: ActivityType[] }).activityTypes || 
                   (data as { items?: ActivityType[] }).items || 
                   data || [];
  ...
  const loadActivities = useCallback(async () => {
    ...
    try {
      ...
      const list = extractActivitiesFromAxiosResponse(resp) as Activity[];
      const envelope = resp?.data;
      const pagination = (envelope as { data?: { pagination?: { total?: number } } })?.data?.pagination;
  ...
  const filtered = useMemo(() => {
    ...
    return (activities || [])
      ...
      .sort((a, b) => new Date(b.ngay_tao || b.ngay_bd || 0).getTime() - new Date(a.ngay_tao || a.ngay_bd || 0).getTime());
