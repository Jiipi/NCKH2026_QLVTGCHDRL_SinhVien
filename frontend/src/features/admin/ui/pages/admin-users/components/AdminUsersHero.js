  ...
  return (
    <div className="relative mb-8 overflow-hidden rounded-3xl">
      ...
      <div
        ...
        style={{ animation: 'neo-float 4s ease-in-out infinite' }}
      ></div>
      <div
        className="absolute bottom-8 left-12 w-16 h-16 bg-yellow-400/30 rounded-full"
        style={{ animation: 'neo-float 3s ease-in-out infinite 0.5s' }}
      ></div>
      ...
      <div className="relative z-10 p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <button onClick={onAddUser} className="group relative inline-flex items-center">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <HeroCard
            ...
            value={`${adminCount}/${teacherCount}/${classMonitorCount}/${studentCount}`}
...
function HeroCard({ icon, label, value, gradient, small }) {
  return (
    <div className="group relative">
      ...
      <div
        className={`relative bg-gradient-to-br ${gradient} border-3 border-black p-4 rounded-2xl transition-transform group-hover:-translate-x-0.5 group-hover:-translate-y-0.5`}
      >
        <div className="flex items-center gap-2 mb-2">{icon}</div>
        <p className={`${small ? 'text-lg sm:text-xl' : 'text-2xl sm:text-3xl'} font-black text-gray-900`}>{value}</p>
