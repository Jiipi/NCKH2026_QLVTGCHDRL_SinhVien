...
export default function ProfilePage(){
  ...
  const [points, setPoints] = React.useState<PointsState>({ total: 0, thisSemester: 0 });
  const [filters, setFilters] = React.useState<FiltersState>({ semester: '' });
  const [activityFilters, setActivityFilters] = React.useState<ActivityFiltersState>({ semester: '', status: '' });
  ...
  const loadPoints = React.useCallback(function(semester: string) {
    setLoading(true);
  ...
  const loadActivities = React.useCallback(function(semester: string, status: string) {
  ...
  async function changePassword(e){
    ...
    try {
      await http.post('/auth/change-password', { currentPassword: form.currentPassword, newPassword: form.newPassword, confirmNewPassword: form.confirmNewPassword });
