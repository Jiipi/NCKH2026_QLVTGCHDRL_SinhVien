...
export default function MonitorApprovalsPage() {
  ...
  useEffect(() => { setPage(1); }, [viewMode, searchTerm, filters]);
  ...
  const loadRegistrations = async () => {
    try {
      setLoading(true);
      const res = await http.get('/core/monitor/registrations', { params: { status: 'all', semester } });
  ...
  const enrichActivityTypes = async (items) => {
    try {
      ...
      console.warn('[MonitorApprovalsPage] enrichActivityTypes failed, returning original items');
      return items;
    }
  };
  ...
  const handleBulkApprove = async () => {
    if (selectedIds.length === 0) { showWarning('Vui lòng chọn ít nhất một đăng ký', 'Chưa chọn đăng ký'); return; }
    const confirmed = await confirm({
    ...
    try {
      ...
      let approvedCount = 0;
      try {
        const res = await http.post('/core/registrations/bulk-approve', { ids: selectedIds });
        approvedCount = res.data?.data?.approved || res.data?.approved || selectedIds.length;
      } catch (_) {
        for (const id of selectedIds) { try { await http.put(`/core/monitor/registrations/${id}/approve`); approvedCount++; } catch (_) {} }
      }
      await loadRegistrations();
      setSelectedIds([]);
  ...
  const handleToggleSelectAll = () => {
    ...
    if (selectedIds.length === pendingRegistrations.length && pendingRegistrations.length > 0) setSelectedIds([]);
    else setSelectedIds(pendingRegistrations.map(r => r.id));
  };
  ...
  const filteredRegistrations = registrations.filter(reg => {
    ...
    if (filters.type) {
      const filterValue = filters.type; const filterId = parseInt(filterValue);
      if (isNaN(filterId)) { const activityTypeName = activity?.loai_hd?.ten_loai_hd || ''; matchesType = activityTypeName.toLowerCase() === filterValue.toLowerCase(); }
      else { const activityTypeId = activity?.loai_hd_id || activity?.loai_hd?.id; const activityId = activityTypeId ? parseInt(activityTypeId) : null; matchesType = activityId !== null && activityId === filterId; }
    }
    if (filters.from) { const fromDate = new Date(filters.from); const activityDate = activity?.ngay_bd ? new Date(activity.ngay_bd) : null; matchesDateFrom = activityDate && activityDate >= fromDate; }
    if (filters.to) { const toDate = new Date(filters.to); toDate.setHours(23,59,59,999); const activityDate = activity?.ngay_bd ? new Date(activity.ngay_bd) : null; matchesDateTo = activityDate && activityDate <= toDate; }
    if (filters.minPoints) { const minPoints = parseFloat(filters.minPoints); if (!isNaN(minPoints)) { const points = parseFloat(activity?.diem_rl) || 0; matchesPointsMin = points >= minPoints; } }
    if (filters.maxPoints) { const maxPoints = parseFloat(filters.maxPoints); if (!isNaN(maxPoints)) { const points = parseFloat(activity?.diem_rl) || 0; matchesPointsMax = points <= maxPoints; } }
    if (filters.mssv) { const mssv = reg.sinh_vien?.mssv || ''; matchesMSSV = mssv.toLowerCase().includes(filters.mssv.toLowerCase()); }
            ...
            <div className="flex items-stretch gap-4 p-4">
              <div className="relative w-32 h-24 flex-shrink-0 rounded-lg overflow-hidden">
                ...
                <div className="absolute top-2 left-2">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${statusColors[registration.trang_thai_dk]}`}>{statusLabels[registration.trang_thai_dk]}</span>
                </div>
                {activity?.diem_rl && (
                  <div className="absolute bottom-2 left-2">
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-amber-500/90 backdrop-blur-sm text-white shadow-sm text-xs font-bold"><Award className="h-3 w-3" />+{activity.diem_rl}</span>
                  </div>
                )}
                {isPending && (
                  <div className="absolute bottom-2 right-2 z-20">
                    <label className="flex items-center gap-1 cursor-pointer bg-white/95 backdrop-blur-sm rounded px-2 py-1 shadow-lg hover:bg-white transition-all" onClick={(e) => e.stopPropagation()}>
                      <input type="checkbox" checked={isSelected} onChange={() => handleToggleSelect(registration.id)} className="w-4 h-4 rounded border-2 cursor-pointer accent-blue-600" onClick={(e) => e.stopPropagation()} />
                    </label>
                  </div>
                )}
              </div>
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  <h3 className="text.base font-bold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-1 mb-2">{activity?.ten_hd || 'Hoạt động'}</h3>
                  <div className="flex items-center gap-2 mb-2">
                    {(() => {
                      const avatar = getUserAvatar(student);
                      return avatar.hasValidAvatar ? (
                        <img src={avatar.src} alt={avatar.alt} className="w-8 h-8 rounded-lg object-cover shadow-sm ring-1 ring-white" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.nextElementSibling.style.display = 'flex'; }} />
                      ) : (
                        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-xs shadow-sm ring-1 ring-white">{avatar.fallback}</div>
                      );
                    })()}
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-gray-400" /><span className="text-gray-600 truncate">{activity?.loai_hd?.ten_loai_hd || 'Chưa phân loại'}</span></div>
                    ...
                    {!isPending && approvedBy && (<div className="flex items-center gap-1.5 col-span-2"><CheckCircle className="h-3.5 w-3.5 text-emerald-500" /><span className="text-gray-600 truncate">Người duyệt: {approvedBy}</span></div>)}
                    {!isPending && rejectedBy && (<div className="flex items-center gap-1.5 col-span-2"><XCircle className="h-3.5 w-3.5 text-rose-500" /><span className="text-gray-600 truncate">Người từ chối: {rejectedBy}</span></div>)}
                  </div>
                </div>
              </div>
              <div className="flex flex-col justify-center gap-2 flex-shrink-0">
                {isPending && registration.canProcess !== false ? (
                  <>
                    <button onClick={() => handleApprove(registration)} disabled={processing} className="flex items-center justify-center gap-1.5 px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg hover:from-emerald-600 hover:to-teal-600 font-medium text-sm shadow-md hover:shadow-lg transition-all duration-200 whitespace-nowrap min-w-[90px] disabled:opacity-50 disabled:cursor-not-allowed"><CheckCircle className="h-4 w-4" />Duyệt</button>
                    <button onClick={() => handleReject(registration)} disabled={processing} className="flex items-center justify-center gap-1.5 px-4 py-2 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-lg hover:from-rose-600 hover:to-pink-600 font-medium text-sm shadow-md hover:shadow-lg transition-all duration-200 whitespace-nowrap min-w-[90px] disabled:opacity-50 disabled:cursor-not-allowed"><XCircle className="h-4 w-4" />Từ chối</button>
                  </>
                ) : (
                  <button onClick={() => { setActivityDetailId(activity?.id); setIsDetailModalOpen(true); }} className="flex items-center justify-center gap-1.5 px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg hover:from-indigo-600 hover:to-purple-600 font-medium text-sm shadow-md hover:shadow-lg transition-all duration-200 whitespace-nowrap min-w-[90px]"><Eye className="h-4 w-4" />Chi tiết</button>
                )}
              </div>
            </div>
          </div>
      ...
      <div className={`group relative h-full ${isSelected ? 'ring-4 ring-blue-500 ring-offset-2' : ''}`}>
        <div className={`relative bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-indigo-300 transition-all duration-300 flex flex-col h-full ${isPending ? 'border-amber-200 shadow-lg shadow-amber-100' : ''}`}>
          <div className="relative w-full h-36 overflow-hidden">
            ...
            <div className="absolute top-2 left-2"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${statusColors[registration.trang_thai_dk]}`}>{statusLabels[registration.trang_thai_dk]}</span></div>
            {activity?.diem_rl && (<div className="absolute bottom-2 right-2"><span className="inline-flex items-center gap-1 px-2 py-1 bg-amber-500/95 backdrop-blur-sm text-white rounded-lg text-xs font-bold shadow-md"><Award className="h-3 w-3" />+{activity.diem_rl}</span></div>)}
            {isPending && (
          ...
          <div className="flex-1 p-4 space-y-3 relative z-10">
            <div>
              <h3 className="text-sm font-bold text-gray-900 line-clamp-2 group-hover:text-indigo-600 transition-colors mb-1.5 leading-tight">{activity?.ten_hd || 'Hoạt động'}</h3>
            ...
            <div className="flex items-center gap-2 p-2 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-100">
              {(() => {
                const avatar = getUserAvatar(student);
                return avatar.hasValidAvatar ? (
                  <img src={avatar.src} alt={avatar.alt} className="w-10 h-10 rounded-lg object-cover shadow-sm ring-1 ring-white" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.nextElementSibling.style.display = 'flex'; }} />
                ) : (
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text.white font-bold text-sm shadow-sm ring-1 ring-white">{avatar.fallback}</div>
                );
              })()}
              <div className="flex-1 min-w-0">
            ...
            <div className="space-y-1.5">
              ...
              {!isPending && approvedBy && (<div className="flex items-center gap-1.5 text-xs"><CheckCircle className="h-3.5 w-3.5 text-emerald-500 flex-shrink-0" /><span className="text-gray-600 truncate">Người duyệt: {approvedBy}</span></div>)}
              {!isPending && rejectedBy && (<div className="flex items-center gap-1.5 text-xs"><XCircle className="h-3.5 w-3.5 text-rose-500 flex-shrink-0" /><span className="text-gray-600 truncate">Người từ chối: {rejectedBy}</span></div>)}
            </div>
          </div>
          <div className="p-3 pt-0 mt-auto flex gap-2">
            {isPending && registration.canProcess !== false ? (
              <>
                <button onClick={() => handleApprove(registration)} disabled={processing} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient.to-r from-emerald-500 to-teal-500 text-white rounded-lg hover:from-emerald-600 hover:to-teal-600 font-medium text-xs shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"><CheckCircle className="h-3.5 w-3.5" />Duyệt</button>
                <button onClick={() => handleReject(registration)} disabled={processing} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient.to-r from-rose-500 to-pink-500 text-white rounded-lg hover:from-rose-600 hover:to-pink-600 font-medium text-xs shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"><XCircle className="h-3.5 w-3.5" />Từ chối</button>
              </>
            ) : (
              <button onClick={() => { setActivityDetailId(activity?.id); setIsDetailModalOpen(true); }} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg hover:from-indigo-600 hover:to-purple-600 font-medium text-xs shadow-md hover:shadow-lg transition-all duration-200"><Eye className="h-3.5 w-3.5" />Chi tiết</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div>
        {effectiveTotal > 0 ? (
          <div className={displayViewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4' : 'space-y-3'}>
            {pageItems.map(reg => (<RegistrationCard key={reg.id} registration={reg} />))}
          </div>
        ) : (
