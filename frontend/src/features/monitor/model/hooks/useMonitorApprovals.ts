...
export function useMonitorApprovals() {
  ...
  const loadRegistrations = useCallback(async () => {
    try {
      ...
      if (result.success && 'data' in result) {
        ...
        setError('error' in result ? result.error : 'Không thể tải danh sách đăng ký');
        setRegistrations([]);
      }
    } catch (err) {
  ...
  const handleApprove = useCallback(async (registration) => {
    try {
      ...
      if (result.success) {
        ...
        showError('error' in result ? result.error : 'Không thể phê duyệt đăng ký');
      }
    } catch (err) {
  ...
  const handleReject = useCallback(async (registration, reason) => {
    try {
      ...
      if (result.success) {
        ...
        showError('error' in result ? result.error : 'Không thể từ chối đăng ký');
      }
    } catch (err) {
  ...
  const handleBulkApprove = useCallback(async () => {
    ...
    const confirmed = await confirm({
      ...
      message: `Bạn có chắc muốn phê duyệt ${selectedIds.length} đăng ký?`,
