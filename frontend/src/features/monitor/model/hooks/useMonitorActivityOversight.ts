...
import { MappedActivity } from '../../../../shared/lib/mappers';
...
/** Response structure for available activities API */
interface AvailableActivitiesResponse {
  items: unknown[];
  total: number;
}

/** Type guard for successful API result with AvailableActivitiesResponse data */
function hasActivitiesData(result: { success: boolean; data?: unknown; error?: string }): result is { success: true; data: AvailableActivitiesResponse } {
  return result.success && result.data !== undefined && typeof result.data === 'object' && result.data !== null;
}

/** Type guard for successful API result with array data */
function hasArrayData(result: { success: boolean; data?: unknown; error?: string }): result is { success: true; data: unknown[] } {
...
export function useMonitorActivityOversight() {
  ...
  const [availableActivities, setAvailableActivities] = useState<MappedActivity[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('cho_duyet');
  const [error, setError] = useState('');
  const [selectedActivity, setSelectedActivity] = useState<MappedActivity | null>(null);
  ...
  const [allActivities, setAllActivities] = useState<MappedActivity[]>([]); // Store all activities for client-side filtering
  ...
  const loadActivities = useCallback(async () => {
    try {
      ...
      if (hasActivitiesData(result)) {
        ...
        setError('error' in result ? result.error : 'Không thể tải danh sách hoạt động');
        setAllActivities([]);
        setActivities([]);
  ...
  const loadActivityTypes = useCallback(async () => {
    try {
      ...
      if (hasArrayData(result)) {
        setActivityTypes((result.data || []) as unknown as ActivityType[]);
      } else {
        setActivityTypes([]);
      }
    } catch (err) {
  ...
  const handleEditActivity = useCallback(async (activity: MappedActivity) => {
    try {
  ...
  const handleSaveActivity = useCallback(async () => {
    ...
    try {
      const diem_rl = Number(selectedActivity.diem_rl) || 0;
      const sl_toi_da = Number(selectedActivity.sl_toi_da) || 0;
      ...
      if (result.success) {
        ...
        showError('error' in result ? result.error : 'Không thể cập nhật hoạt động', 'Lỗi cập nhật');
      }
    } catch (err: any) {
  ...
  const handleDeleteActivity = useCallback(async (activity: MappedActivity) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showError('error' in result ? result.error : 'Không thể xóa hoạt động', 'Lỗi xóa hoạt động');
      }
    } catch (err: any) {
  ...
  const handleRegister = useCallback(async (activityId: string, activityName: string) => {
    const confirmed = await confirm({
    ...
    try {
      ...
      if (result.success && 'data' in result) {
        ...
        showError('error' in result ? result.error : 'Đăng ký thất bại');
      }
    } catch (err: any) {
  ...
  const handleShowQR = useCallback((activity: MappedActivity) => {
  ...
  const parseDateSafe = useCallback((d: string | Date | null | undefined) => {
    try {
  ...
  const hasEndedByTime = useCallback((a: MappedActivity | null) => {
    const end = parseDateSafe(a?.ngay_kt);
    if (!end) return false;
    return Date.now() > (end.getTime() + 60 * 1000);
  }, [parseDateSafe]);

  const getDisplayStatus = useCallback((a: MappedActivity | null) => {
    if (!a) return 'cho_duyet';
    if (a.trang_thai === 'da_duyet' && hasEndedByTime(a)) return 'ket_thuc';
    return a.trang_thai;
  }, [hasEndedByTime]);

  const isAvailable = useCallback((a: MappedActivity | null) => {
    if (!a) return false;
