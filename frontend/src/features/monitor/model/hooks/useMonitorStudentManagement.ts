...
export function useMonitorStudentManagement() {
  ...
  const loadStudents = useCallback(async () => {
    try {
      ...
      if (result.success && 'data' in result) {
        ...
        setError('error' in result && result.error ? result.error : 'Không thể tải danh sách sinh viên');
        setStudents([]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (!semester && currentSemester) {
      ...
      try { sessionStorage.setItem('monitor_student_semester', currentSemester); } catch { /* ignore */ }
      return;
    }
    ...
    if (semester && !hasSemesterInOptions && fallbackSemester && fallbackSemester !== semester) {
      ...
      try { sessionStorage.setItem('monitor_student_semester', fallbackSemester); } catch { /* ignore */ }
    }
