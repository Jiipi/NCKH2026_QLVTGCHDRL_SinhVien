...
/** Interface for so_sanh_lop (class comparison) data */
interface SoSanhLop {
  my_rank_in_class: number;
  total_students_in_class: number;
}

/** Interface for student dashboard UI data */
interface StudentDashboardUI {
  tong_quan: TongQuan;
  so_sanh_lop: SoSanhLop;
  hoat_dong_sap_toi: unknown[];
}
...
export default function useMonitorDashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (monitorResult.success && 'data' in monitorResult) {
        setMonitorDashboardData(monitorResult.data);
      } else {
        console.error('[useMonitorDashboard] Monitor dashboard error:', 'error' in monitorResult ? monitorResult.error : 'Unknown error');
        setMonitorDashboardData(null);
      }

      // Set student dashboard data (for personal stats)
      if (studentResult.success && 'data' in studentResult) {
        setStudentDashboardData(studentResult.data);
      } else {
        console.error('[useMonitorDashboard] Student dashboard error:', 'error' in studentResult ? studentResult.error : 'Unknown error');
        setStudentDashboardData(null);
      }

      // Set my activities data
      if (activitiesResult.success && 'data' in activitiesResult) {
        setMyActivitiesData(activitiesResult.data || []);
      } else {
        console.error('[useMonitorDashboard] Activities error:', 'error' in activitiesResult ? activitiesResult.error : 'Unknown error');
        setMyActivitiesData([]);
      }

      // Set profile data
      if (profileResult.success && 'data' in profileResult) {
        setProfileData(profileResult.data);
      } else {
        console.error('[useMonitorDashboard] Profile error:', 'error' in profileResult ? profileResult.error : 'Unknown error');
        setProfileData(null);
      }
    } catch (err) {
  ...
  const studentDashboard = useMemo<StudentDashboardUI>(() => {
    ...
    return mapStudentDashboardToUI(studentDashboardData) as StudentDashboardUI;
