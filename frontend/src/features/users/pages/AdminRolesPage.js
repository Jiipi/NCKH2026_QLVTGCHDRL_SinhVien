    ...
    <div style={{ minHeight: '100vh', padding: 24, background: 'linear-gradient(to bottom right, #f0f9ff, white, #faf5ff)' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ position: 'relative' }}>
                    ...
                    <div style={{ 
                      ...
                      height: 12, 
                      borderRadius: '50%', 
                      background: u.trang_thai === 'hoat_dong' ? '#22c55e' : '#ef4444',
                ...
                <div>
                  <span style={{ 
                    ...
                    padding: '6px 12px', 
                    borderRadius: 9999, 
                    background: u.trang_thai === 'hoat_dong' ? '#dcfce7' : '#fee2e2', 
                    color: u.trang_thai === 'hoat_dong' ? '#166534' : '#991b1b',
                    fontWeight: 700,
                    border: `2px solid ${u.trang_thai === 'hoat_dong' ? '#86efac' : '#fca5a5'}`
                  }}>
                    {u.trang_thai === 'hoat_dong' ? <CheckCircle size={14} /> : <XCircle size={14} />}
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <Users size={16} />
          <span>
            Hiển thị <strong style={{ color: '#111827' }}>{users.length ? (userPage - 1) * userLimit + 1 : 0} - {Math.min(userPage * userLimit, userTotal)}</strong> / <strong style={{ color: '#111827' }}>{userTotal}</strong> người dùng
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {/* Page Limit Selector */}
          ...
          <button 
            ...
            style={{ 
              ...
              borderRadius: 10,
              border: '2px solid #e5e7eb',
              background: userPage <= 1 ? '#f9fafb' : 'white',
              color: userPage <= 1 ? '#d1d5db' : '#374151',
          ...
          <div style={{ 
            ...
            {userPage} / {Math.max(1, Math.ceil(userTotal / userLimit))}
          </div>

          {/* Next Button */}
          <button 
            disabled={userPage >= Math.ceil(userTotal / userLimit)} 
            onClick={() => setUserPage(p => Math.min(Math.ceil(userTotal / userLimit), p + 1))} 
            style={{ 
              padding: '8px 16px',
              borderRadius: 10,
              border: '2px solid #e5e7eb',
              background: userPage >= Math.ceil(userTotal / userLimit) ? '#f9fafb' : 'white',
              color: userPage >= Math.ceil(userTotal / userLimit) ? '#d1d5db' : '#374151',
              cursor: userPage >= Math.ceil(userTotal / userLimit) ? 'not-allowed' : 'pointer',
              fontSize: 13,
              fontWeight: 600,
              transition: 'all 0.2s'
            }}
