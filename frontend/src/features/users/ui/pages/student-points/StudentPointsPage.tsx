...
export default function StudentPoints() {
  ...
  const [semester, setSemester] = useState(() => {
    try {
      return sessionStorage.getItem('student_points_semester') || '';
    } catch {
      return '';
    }
  });
  ...
  useEffect(() => {
    if (semester) {
      try {
        sessionStorage.setItem('student_points_semester', semester);
      } catch {
        // ignore
      }
    }
  ...
  const loadData = async () => {
    try {
      ...
      const params: { semester?: string; hoc_ky?: string; nam_hoc?: string } = {};
      if (semester) {
      ...
      // Load points summary
      const summaryResponse = await http.get('/core/points/summary', { params });
      ...
      // Load points detail
  const detailResponse = await http.get('/core/points/detail', { params });
  ...
  const attendanceResponse = await http.get('/core/points/attendance-history', { params });
                  ...
                  <div className="space-y-3">
                    {pointsSummary.diem_theo_tieu_chi && Object.entries(pointsSummary.diem_theo_tieu_chi).map(([key, value]) => (
                      <div key={key} className="flex justify-between items-center py-2 border-b border-gray-100">
                        ...
                        <span className="font-medium">{String(value)} điểm</span>
                      </div>
                    ))}
                  </div>
                </div>
