    ...
    <div className="relative overflow-hidden rounded-[2rem] border border-white/60 bg-white/60 p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/55 dark:shadow-black/20">
      ...
      <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-black tracking-[-0.04em] text-slate-950 dark:text-white sm:text-5xl">Chứng nhận của tôi</h1>
          <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300">Tổng hợp các chứng nhận hoạt động đã hoàn thành.</p>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:min-w-[280px]">
          <GlassMetric icon={Award} label="Chứng nhận" value={certificates.length} tone="text-indigo-600 dark:text-indigo-300" />
          <GlassMetric icon={Trophy} label="Tổng điểm" value={totalPoints.toFixed(1)} tone="text-amber-600 dark:text-amber-300" />
        </div>
      </div>
    </div>
  );
