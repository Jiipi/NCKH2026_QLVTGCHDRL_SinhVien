...
const EditActivityModal: FC<EditActivityModalProps> = ({ 
  isOpen, 
  onClose, 
  activity, 
  onSave 
  ...
  const [localActivity, setLocalActivity] = useState<Activity | null>(activity);
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setLocalActivity(prev => prev ? { ...prev, [name]: value } : null);
  };

  const handleFileChange = (field: string, urls: string[]) => {
    setLocalActivity(prev => prev ? { ...prev, [field]: urls } : null);
  };
            ...
            {editMode && (
                <div className="md:col-span-2">
                    <FileUpload
                        ...
                        onChange={(urls: string[]) => handleFileChange('hinh_anh', urls)}
                    />
                </div>
            )}
            {editMode && (
                <div className="md:col-span-2">
                    <FileUpload
                        ...
                        onChange={(urls: string[]) => handleFileChange('tep_dinh_kem', urls)}
                    />
                </div>
            )}
          </div>
