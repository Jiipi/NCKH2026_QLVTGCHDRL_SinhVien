...
export default function useForgotPassword() {
  ...
  const handleSendOTP = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError('');
    setSuccess('');
    ...
    setIsLoading(true);
    try {
      const result = await authApi.forgotPassword(email) as ApiResult;
      ...
      const error = err as ForgotPasswordApiError;
  ...
  const handleVerifyCode = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError('');
    setSuccess('');
    ...
    setIsLoading(true);
    try {
      const result = await authApi.verifyForgotPasswordCode(email, code) as ApiResult;
      ...
      const error = err as ForgotPasswordApiError;
      const errMsg = error?.response?.data?.message || 'Mã không đúng hoặc đã hết hạn.';
      if (errMsg.includes('không hợp lệ')) {
        setError('Mã không hợp lệ hoặc đã hết hạn. Nếu bạn không nhận được email, có thể địa chỉ email chưa được đăng ký trong hệ thống.');
      } else {
        setError(errMsg);
  ...
  const handleResetPassword = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const error = err as ForgotPasswordApiError;
      const errMsg = error?.response?.data?.message || 'Không thể đặt lại mật khẩu.';
      setError(errMsg);
