...
/** Hook options */
interface UseTeacherApprovalsOptions {
    ...
    try {
      ...
      if (activeTab === 'pending') {
        ...
        if (result.success && 'data' in result) {
          ...
          const errorMsg = 'error' in result ? result.error : 'Không thể tải lịch sử phê duyệt';
          console.error('[useTeacherApprovals] Load history error:', errorMsg);
          setError(errorMsg);
          setActivitiesData([]);
        }
      }
