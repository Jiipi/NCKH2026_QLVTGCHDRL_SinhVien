  ...
  const filteredActivities = useMemo(() => {
    ...
    return activities
      .filter((activity) => {
        ...
        const matchesType = !filters.type || String((activity as any).loai_hd_id || (activity as any).loai_hd?.id) === String(filters.type);
      ...
      .sort((a, b) => {
        // Cast to any to handle optional API fields not in strict type definition
        const aAny = a as any;
        const bAny = b as any;
        const getTimestamp = (item: any) =>
          new Date(item.ngay_cap_nhat || item.updated_at || item.updatedAt || item.ngay_tao || item.createdAt || item.ngay_bd || 0).getTime();

        switch (sortBy) {
          case 'oldest':
            return getTimestamp(aAny) - getTimestamp(bAny);
          case 'name-az':
          ...
          case 'newest':
          default:
            return getTimestamp(bAny) - getTimestamp(aAny);
        }
      });
