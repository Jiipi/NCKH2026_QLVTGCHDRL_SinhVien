...
export default function useTeacherDashboard({ semester, classId }) {
  ...
  useEffect(() => {
    ...
    async function loadDerivedStats() {
      ...
      try {
        ...
        const activitiesList = (activitiesRes.success && 'data' in activitiesRes)
          ? dedupeById(activitiesRes.data?.items || [])
          : [];
        const totalActivities = (activitiesRes.success && 'data' in activitiesRes)
          ? (activitiesRes.data?.total ?? activitiesList.length)
          : 0;

        const pendingRegistrationsList = (pendingRegistrationsListRes.success && 'data' in pendingRegistrationsListRes)
        ...
        const approvedRegistrationsList = (approvedRegistrationsListRes.success && 'data' in approvedRegistrationsListRes)
        ...
        const reportOverview = (reportsRes.success && 'data' in reportsRes) ? reportsRes.data?.overview || {} : {};
  ...
  const stats = useMemo(() => {
    const base: DashboardSummary = dashboard.summary || {};
  ...
  const handleApprove = useCallback(async (id) => {
    try {
      ...
      if (result.success) {
        ...
        setError('Không thể phê duyệt hoạt động');
      }
    } catch (err) {
  ...
  const handleReject = useCallback(async (id, reason) => {
    try {
      ...
      if (result.success) {
        ...
        setError('Không thể từ chối hoạt động');
      }
    } catch (err) {
