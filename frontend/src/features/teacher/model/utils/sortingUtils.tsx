 ...
 * @returns {Array} Sorted array
 */
export function sortItems<T extends Record<string, unknown>>(items: T[], sortBy: string, config: SortConfig = {}): T[] {
  const {
    nameGetter = (item: Record<string, unknown>) => (item.name as string) || (item.ho_ten as string) || '',
    pointsGetter = (item: Record<string, unknown>) => (item.diem_rl as number) || (item.points as number) || 0,
