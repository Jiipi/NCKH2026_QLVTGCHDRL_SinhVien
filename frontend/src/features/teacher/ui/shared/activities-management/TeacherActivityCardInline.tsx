...
interface TeacherActivityCardInlineProps {
  activity: Activity;
  viewMode?: 'list' | 'grid';
  isWritable?: boolean;
...
export default function TeacherActivityCardInline({
  ...
  onEdit,
  onDelete
}: TeacherActivityCardInlineProps) {
  const {
    startDate,
    endDate,
    status,
...
function buildActivityMeta(activity: Activity = {} as Activity) {
  const parseDateSafe = (value: string | Date | undefined | null): Date | null => {
    try {
  ...
  const statusKey = activity.trang_thai as keyof typeof statusConfig;
  const status = statusConfig[statusKey] || statusConfig.da_duyet;
...
function InfoRow({ icon: Icon, label, subLabel }: InfoRowProps) {
...
function ActionButton({ onClick, icon: Icon, label, gradient, variant, fullWidth, disabled = false, disabledTitle }: ActionButtonProps) {
