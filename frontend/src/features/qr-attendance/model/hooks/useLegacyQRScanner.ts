...
export function useLegacyQRScanner() {
    ...
    try {
      ...
      if (videoTrack) {
        ...
        if (Object.keys(constraints).length > 0) {
          try { await videoTrack.applyConstraints({ advanced: [constraints] } as MediaTrackConstraints); } catch (_) {}
        }
      }
      if (videoRef.current) {
  ...
  async function setupZXing() {
    ...
    try {
      ...
      zxingCleanupRef.current = () => { zxingControlsRef.current = null; try { controls?.stop?.(); } catch(_) {} try { (reader as any)?.dispose?.(); } catch(_) {} };
    } catch (_) {
      zxingReaderRef.current = null;
    }
  }
